...
from dynamics.FlightDynamics import ForcesMoments
...
class ComputeTrim:
    ...
    def __init__(self):
        ...
        self.forces_mom = ForcesMoments()
        self.mav = FlightDynamics()
    ...
    def compute_trim_states_input(self, x, Va, Y, R):
        ...
        d_e = (((jxz * (p ** 2 - r ** 2) + (jx - jz) * p * r) / (
                    0.5 * rho * (Va ** 2) * c * S_wing)) - C_m_0 - C_m_alpha * alpha - C_m_q * (
                           (c * q) / (2 * Va))) / C_m_delta_e

        d_t = np.sqrt(((2 * mass * (-r * v + q * w + gravity * sin(theta)) - rho * (Va ** 2) * S_wing * (
                    C_X + C_X_q * ((c * q) / (2 * Va)) + C_X_delta_e * d_e)) / (
        ...
        temp_2 = np.array([[((-gamma1 * p * q + gamma2 * q * r) / (
                    0.5 * rho * (Va ** 2) * S_wing * b)) - C_ell_0 - C_ell_beta * beta - C_ell_p * (
                                        (b * p) / (2 * Va)) - C_ell_r * ((b * r) / (2 * Va))],
                           [((-gamma7 * p * q + gamma1 * q * r) / (
                                       0.5 * rho * (Va ** 2) * S_wing * b)) - C_n_0 - C_n_beta * beta - C_n_p * (
    ...
    def compute_trim_cost(self, x, Va, Y, R):
        ...
        f_m,VaTemp = self.forces_mom.compute(X_trim, U_trim)
        ...
        states_dot = self.mav.f(X_trim, f_m)  #
